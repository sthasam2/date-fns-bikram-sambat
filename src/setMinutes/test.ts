...
describe("setMinutes", () => {
	it("sets the minutes", () => {
		...
		expect(result).toEqual(
			/* 2071/04/16 */ new Date(2014, 8 /* Sep */, 1, 11, 45, 40),
		);
	});

	it("accepts a timestamp", () => {
		const result = setMinutes(
			/* 2071/04/16 */ new Date(2014, 8 /* Sep */, 1, 11, 30).getTime(),
			5,
		);
		expect(result).toEqual(
			/* 2071/04/16 */ new Date(2014, 8 /* Sep */, 1, 11, 5),
		);
	});

	it("does not mutate the original date", () => {
		const date = /* 2071/04/16 */ new Date(2014, 8 /* Sep */, 1, 11, 30);
		setMinutes(date, 15);
		expect(date).toEqual(
	...
	describe("context", () => {
		...
		it("resolves the context date type", () => {
			const result = setMinutes(/* 2071/04/16 */ "2014-09-01T00:00:00Z", 45, {
				in: tz("Asia/Tokyo"),
			});
