...
describe("setDay", () => {
	it("sets the day of the week", () => {
		const result = setDay(/* 2071/04/16 */ new Date(2014, 8 /* Sep */, 1), 0);
	...
	describe("the day index is more than 6", () => {
		it("sets the day of the next week", () => {
			const result = setDay(/* 2071/04/16 */ new Date(2014, 8 /* Sep */, 1), 8);
			expect(result).toEqual(/* 1393/6/16 */ new Date(2014, 8 /* Sep */, 7));
		});
	...
	it("returns `Invalid Date` if the given amount is NaN", () => {
		const result = setDay(/* 2071/04/16 */ new Date(2014, 8 /* Sep */, 1), NaN);
		expect(result instanceof Date && isNaN(result.getTime())).toBe(true);
	});
	...
	describe("context", () => {
		...
		it("resolves the context date type", () => {
			const result = setDay(/* 2071/04/16 */ "2014-09-01T00:00:00Z", 15, {
				in: tz("Asia/Tokyo"),
			});
