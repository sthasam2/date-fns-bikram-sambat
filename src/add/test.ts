...
describe("add", () => {
	it("adds the values from the given object", () => {
		const result = add(
			/* 2071/04/16 */ new Date(2014, 8 /* Sep */, 1, 10, 19, 50),
			{
				years: 2,
				months: 9,
				weeks: 1,
	...
	it("returns the same date object when passed empty duration values", () => {
		const result = add(
			/* 2071/04/16 */ new Date(2014, 8 /* Sep */, 1, 10).getTime(),
			{
				years: undefined,
				months: undefined,
				weeks: undefined,
		...
		expect(result).toEqual(/* 2071/04/16 */ new Date(2014, 8 /* Sep */, 1, 10));
	});
	...
	it("accepts a timestamp", () => {
		...
		expect(result).toEqual(/* 2071/04/16 */ new Date(2014, 8 /* Sep */, 1, 14));
	});

	it("does not mutate the original date", () => {
		const date = /* 2071/04/16 */ new Date(2014, 8 /* Sep */, 1, 10);
		add(date, { hours: 4 });
		expect(date).toEqual(/* 2071/04/16 */ new Date(2014, 8 /* Sep */, 1, 10));
	});
	...
	describe("context", () => {
		...
		it("resolves the context date type", () => {
			const result = add(
				/* 2071/04/16 */ "2014-09-01T00:00:00Z",
				{ days: 10 },
				{ in: tz("Asia/Tokyo") },
			);
